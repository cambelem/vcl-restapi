...
function AJsubmitRenameNode() {
	...
	$_newname = mysqli_real_escape_string($newname);
	$query = "SELECT id "
	       . "FROM privnode "
...
function userLookup() {
	...
	print "</FORM><br>\n";
	if(! empty($userid)) {
		$esc_userid = mysqli_real_escape_string($userid);
		...
		if($mode == 'userid') {
			...
			$tmp = explode(',', $userid);
			$last = mysqli_real_escape_string(trim($tmp[0]));
			$first = mysqli_real_escape_string(trim($tmp[1]));
